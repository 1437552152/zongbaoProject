window.vxgplayer = function(id, options_) {
  ...
  if (!window.vxgplayer.players[id]) {
    ...
    window.vxgplayer.players[id] = new (function(id, opts) {
      ...
      var el_controls_zoom_position = self.player.getElementsByClassName(
      ...
      var el_fullscreen = self.player.getElementsByClassName('vxgplayer-fullscreen');
      var el_takescreenshot = self.player.getElementsByClassName('vxgplayer-takescreenshot')[0];
      var el_screenshot_loading = self.player.getElementsByClassName(
      ...
      var el_zoomPositionCursor = self.player.getElementsByClassName(
      ...
      self.set_controls_display = function(val) {
        ...
        if (el_fullscreen && el_fullscreen.length == 2) {
          el_fullscreen[0].style.display = val == 'none' ? '' : 'none';
        }
      ...
      self.loadSettings = function() {
        ...
        if (self.player.hasAttribute('useragent-prefix')) {
          ...
          self.module.command(
            'setuseragent',
            self.options.useragent_prefix + ' ' + navigator.userAgent
          );
        }
      ...
      self.connectToApp = function() {
        ...
        self.m.port.onDisconnect.addListener(function() {
          if (self.m.debug) console.log('disconnected portName=' + self.m.portName);
          self.module.command('stopnativeclient', '@' + self.m.portName);

          self.m.port = undefined;
        });

        if (self.m.debug) console.log('connected portName=' + self.m.portName);
      ...
      self.handleMessage = function(msgEvent) {
        ...
        if (msgEvent.data[0] != undefined && msgEvent.data[0].charAt(0) == '@') {
          ...
          if (self.m.port != undefined) self.m.port.postMessage(msgEvent.data);
        ...
        // Player listener2: Skip picture: 170, frame_duration: 41, latencyms: 0<=frames_buf:0, b: 0, p: 0
          ...
          if (self.m.debug) console.log('=VERSION_APP ' + self.m.versionapp);
          self.playerDidLoad();
          ...
          if (0 != snap_status.indexOf('ERROR')) {
            ...
            if (snap_pts != undefined) self.m.snapshotPTS = snap_pts;
            ...
            window.requestFileSystem(
              ...
              function(e) {
                console.error(
                  '[VXGPLAYER] TAKE_SNAPSHOT requestFileSystem window.TEMPORARY FAILED'
                );
              }
            );
          }
          ...
          if (self.m.handlerError) self.m.handlerError(self);
          ...
          if (self.m.debug)
            console.log(
              '=MEDIA_ERR_DECODER ' + self.m.lastErrorDecoder + ' ' + (self.m.lastErrorDecoder == 0)
                ? 'Decoder resumed'
                : ''
            );
          ...
          if (self.onStateChangeCallback) self.onStateChangeCallback(self.m.vxgReadyState);
      ...
      self.play = function() {
        ...
        if (self.m.debug)
          console.log(
            'self.play self.m.url=' + self.m.url + ' self.m.is_opened=' + self.m.is_opened
          );
        ...
        el_play.style.display = 'none';
        if (self.m.vxgReadyState != 4)
          //not paused=>play, show progress
          el_loader.style.display = 'inline-block';
      ...
      self.setNewZoom = function(v) {
        ...
        if (self.currentZoom != v) {
          ...
          el_zoomProgress.className = el_zoomProgress.className.replace(
            /zoom\d+x/g,
            'zoom' + Math.ceil(self.currentZoom) + 'x'
          );
          el_controls_zoom_position.style.display = self.currentZoom == 10 ? 'none' : '';
      ...
      if (el_fullscreen) {
        for (var i = 0; i < el_fullscreen.length; i++) {
          el_fullscreen[i].onclick = self.fullscreen;
        }
      }
...
window.vxgplayer.showGlobalErrorMessage = function(id, html) {
  ...
  var width = 640;
  var height = 480;
  width = parseInt(player.getAttribute('twidth'), 10) || width;
  height = parseInt(player.getAttribute('theight'), 10) || height;
...
window.vxgplayer.showNotInstalled = function(id) {
  vxgplayer.showGlobalErrorMessage(
    ...
    '' +
      ...
      '缺少插件:<br/><a href="/vxgplayer" target="_blank">本地解决方案</a>' +
      '	<br/><br/> or webstore <a href="https://chrome.google.com/webstore/detail/vxg-media-player/hncknjnnbahamgpjoafdebabmoamcnni" target="_blank">VXG Media Player</a>' +
      '<br/><br/>	<a href="https://www.videoexpertsgroup.com/player_start/" ' +
...
window.vxgplayer.showAvailableInChrome = function(id) {
  vxgplayer.showGlobalErrorMessage(
    ...
    '' +
      '<div class="vxgplayer-unsupport">' +
      '	<div class="vxgplayer-unsupport-content">' +
      ' Available in <a href="https://www.google.com/chrome/" target="_blank">只能运行在PC的Chrome浏览器中,请先下载chrome浏览器</a>' +
      '	</div>' +
      '</div>'
  );
};
...
window.vxgplayer.initOptions = function(self, options) {
  ...
  self.m.audio_parameter =
    self.player.hasAttribute('audio') && parseInt(self.player.getAttribute('audio', 10)) == 0
      ? '0'
      : '1';
  self.m.mute_parameter =
    self.player.hasAttribute('mute') && parseInt(self.player.getAttribute('mute', 10)) == 1
      ? '1'
      : '0';
};
...
window.vxgplayer.initVolumeControls = function(self, onloadsettings) {
  ...
  el_volumeProgress.className = el_volumeProgress.className.replace(
    /vol\d+/g,
    'vol' + Math.ceil(self.m.volume * 10)
  );
  ...
  self.applyVolume = function() {
    if (self.isMute()) {
      ...
      el_volumeProgress.className = el_volumeProgress.className.replace(
        /vol\d+/g,
        'vol' + Math.floor(self.m.volume * 10)
      );
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  console.log(
    'vxgplayer browserSupportsPluginPnacl: ' + window.vxgplayer.browserSupportsPluginPnacl()
  );
