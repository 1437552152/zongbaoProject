...
@connect(({ operationWorkOrder, loading }) => ({
  operationWorkOrder,
  loading: loading.models.operationWorkOrder,
}))
class WorkOrderList extends PureComponent {
  ...
  fetchOrderTypes = () => {
    ...
    dispatch({
      type: 'operationWorkOrder/fetchOrderTypes',
    });
  };
  ...
  fetchOrderStates = () => {
    const { dispatch } = this.props;
    dispatch({
      type: 'operationWorkOrder/fetchOrderStates',
    });
  };
  ...
  fetchOrderAreaTree = (type, state, beginDate, endDate) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'operationWorkOrder/fetchOrderAreaTree',
      payload: {
        type: type || '',
        state: state || '',
        beginDate: beginDate || '',
  ...
  fetchOrderList = (areaId, type, state, beginDate, endDate, page) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'operationWorkOrder/fetchOrderList',
      payload: {
        areaId,
        type: type || '',
        state: state || '',
  ...
  stateDescription = code => {
    ...
    const {
      operationWorkOrder: { orderStates },
    } = this.props;
  ...
  treePath = () => {
    const {
      operationWorkOrder: { orderAreaTree },
    } = this.props;
  ...
  renderContent = () => {
    ...
    const {
      operationWorkOrder: { orderList, orderListLength },
    } = this.props;
    const location = this.treePath();
    return (
      <StandardCard src="./assets/menu/time-s.png" full className={styles.content}>
  ...
  renderFilter = () => {
    const {
      operationWorkOrder: { orderTypes, orderStates },
    } = this.props;
    return (
