/**
 * Created by skyinno on 2019/5/7.跟区域相关的视频监控历史纪录
 */
...
import styles from './VideoMonitorListPage.less';
import { VideoMonitorByRegionListPageColumns } from '../ColumnsPlubic';
...
class VideoMonitorByRegionListPage extends PureComponent {
  state = { 
    selectPersons: [], 
    visible: false,
    visibleL:false,
  ...
  componentDidMount() {
    ...
    //   firecontrolhistory,
    // } = this.props;
    // // const values={page:1,pageSize:10,areaId:1};
    // const values = {
    //   page: firecontrolhistory.data.pagination.current,
    //   pageSize: firecontrolhistory.data.pagination.pageSize,
    //   areaId: id,
    //   Q: 'type_S_LK=003',
    // };
...
onStatus = (id,listData) => {
  ...
  render() {
    ...
    const tableProps = {
      rowKey: 'id',
      bordered: true,
      columns: VideoMonitorByRegionListPageColumns({ onDelete: this.onDelete, onDetail: this.onDetail, onStatus: this.onStatus, onPic: this.onPic, onPicStatus:this.onPicStatus,lookPic:this.lookPic }),
      data:{
        list:data,
        pagination: {
         total,
...
export default VideoMonitorByRegionListPage;
